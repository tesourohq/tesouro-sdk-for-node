...
async function parallelQueryExecution() {
  ...
  try {
    ...
    const promises = [
      ...
      // Query 5: More transaction summaries
      client.paymentTransactionSummaries({
        input: {
          paging: { skip: 0, take: 5 },
          where: {
...
async function mixedQueryTypes() {
  ...
  try {
    ...
    const requests = [
      ...
      {
        name: 'Recent Transactions (Last 3 days)',
        promise: client.paymentTransactions({
          input: {
            paging: { skip: 0, take: 3 },
            where: {
      ...
      {
        name: 'Summary with Different Range',
        promise: client.paymentTransactionSummaries({
          input: {
            paging: { skip: 0, take: 3 },
            where: {
    ...
    results.forEach((result, index) => {
      ...
      if (result.status === 'fulfilled') {
        ...
        if ('paymentTransactions' in data) {
          ...
          console.log(`   📊 Retrieved ${data.paymentTransactionSummaries.items.length} summaries`);
        }
 ...
 * Demonstrates how to handle failures in concurrent scenarios
 */
async function resilientConcurrentRequests() {
  ...
  try {
    ...
    console.log(`🚀 Starting resilient concurrent requests (max ${maxRetries} retries)...`);
